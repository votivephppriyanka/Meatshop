...
class ControllerAccountRegisters extends Controller {
	...
	public function registrationapi() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateapi()) {
			$customer_id = $this->model_account_customer->addCustomerapi($this->request->post);
			...
			if (isset($customer_id)) {
				...
				$subject = "Meat Shop : Email Verification";
				$txt = "Your email verification otp : ".$code."";
				$header = "From: ".$this->config->get('config_email')."";
	...
	public function otpverify(){
		...
		$this->load->model('account/customer');
		$result=$this->model_account_customer->verifyotpcode($email,$code);
		if ($result=='1') {
			$updatecode="";
			$status="1";
			$this->model_account_customer->editCode($email, $updatecode);
			$this->model_account_customer->customerapproved($email, $status);
			$json = array("status" => 1, "success" => "Your account has verify successfully.");
		}else{
			$json = array("status" => 0, "error" => "You have entered incorrect otp");
		}
		header('Content-type: application/json');
		echo json_encode($json);
	}
